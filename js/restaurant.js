...
function fireRule(rule) {
  ...
  */
  ...
  try {
    $(".table").xpathEvaluate(rule).not(baseTable);
  }
  catch(err) {
    rule = null;
  }
...
//  var levelSelected = $(".table").find(level.selector).not(baseTable); // What the person finds
  var ruleSelected = $(".table").xpathEvaluate(rule).not(baseTable);   
  var levelSelected = $(".table").xpathEvaluate(level.selector).not(baseTable);
